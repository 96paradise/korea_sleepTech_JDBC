...
// 4. SQL 실헹(executeQuery, executeUpdate)
...
// 1) executeQuery
// : Select문과 같은 쿼리문 실행 시 사용
// - 쿼리 실행 후 결과를 ResultSet 객체로 변환: 반환된 ResultSet 객체를 통해 결과를 가져옴

// 2) executeUpdate
// : Insert, Update, Delete와 같은 dml(data manipulation language)에서 실핼 결과로 영향 받은 레코드 수를 반환
// - 반환 타입 int
...
// 2) rs.getInt("칼럼명"), getString("컬럼명"), getDate("칼럼명")
